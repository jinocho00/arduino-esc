...
void setup() {
  ...
  //DDRB  |= 0x0E;           // Configure pins 8 as outputs
  //PORTB  &= 0x31;          //B00110001    D9, D10 and D11 to LOW
  DDRD  |= 0x93;            // Configure pins 2(PD1), 3(PD0), 4(PD4) and 6(PD7) as outputs  
  PORTD &= ~0x93;           // pins 2(PD1), 3(PD0), 4(PD4) and 6(PD7) set to LOW
  DDRC  |= 0x40;            // Configure pin 5(PC6) as output
  PORTC &= ~0x40;           // pin 5(PC6) set to LOW
  DDRB  &= ~0x10;           // Configure pin 8(PB4) as input
  DDRB  |= 0x20;            // Configure pin 9(PB5) as output
  PORTB &= ~0x20;           // pin 8(PB4) set to LOW
  ...
  //Set D9 (PWM in) to trigger interrupt
  PCICR |= (1 << PCIE0);    //enable PCMSK0 scan                                                 
  //PCMSK0 |= (1 << PCINT0);  //Set pin D8 trigger an interrupt on state change. 
  PCMSK0 |= (1 << PCINT4);   //Set pin 8 trigger an interrupt on state change. 
...
void BEMF_A_RISING(){  
  ...
  ADMUX = 5;                // Select A2(ADC5) as comparator negative input
...
void BEMF_B_RISING(){
  ADCSRA = (0 << ADEN);     // Disable the ADC module
  ADCSRB = (1 << ACME);     // Enable MUX select for negative input of comparator
  ADMUX = 6;                // Select A1(ADC6) as comparator negative input
  ACSR |= 0x03;             // Set interrupt on rising edge
}
void BEMF_B_FALLING(){
  ADCSRA = (0 << ADEN);     // Disable the ADC module
...
void BEMF_C_RISING(){
  ADCSRA = (0 << ADEN);     // Disable the ADC module
  ADCSRB = (1 << ACME);     // Enable MUX select for negative input of comparator
  ADMUX = 7;                // Select A0(ADC7) as comparator negative input
  ACSR |= 0x03;             // Set interrupt on rising edge
}
void BEMF_C_FALLING(){
  ADCSRA = (0 << ADEN);     // Disable the ADC module
...
//D9 PWM and D4 HIGH.  
void AH_BL(){
  //PORTD &= ~0x02;         //Pin 2(PD1) set to LOW
  //PORTD |= 0x10;          //Pin 4(PD4) set to HIGH
  //PORTD &= ~0x80;         //Pin 6(PD7) set to LOW
  //TCCR2A =  0;            //OC2A - D11 normal port. 
...
void AH_CL(){
  //PORTD &= ~0x02;         //Pin 2(PD1) set to LOW
  //PORTD &= ~0x10;         //Pin 4(PD4) set to LOW
  //PORTD |= 0x80;          //Pin 6(PD7) set to HIGH
  //TCCR2A =  0;            //OC2A - D11 normal port. 
  //TCCR1A =  0x81;         //OC1A - D9 compare match noninverting mode, downcounting ,PWM 8-bit  
  LOW_LEVEL(0,0,1);
  HIGH_MODE(1,0,0);
...
//D10 PWM and D1 HIGH
void BH_AL(){  
  //PORTD |= 0x02;          //Pin 2(PD1) set to HIGH
  //PORTD &= ~0x10;         //Pin 4(PD4) set to LOW
  //PORTD &= ~0x80;         //Pin 6(PD7) set to LOW
  //TCCR2A =  0;            // OC2A - D11 normal port. 
  //TCCR1A =  0x21;         //OC1B - D10 compare match noninverting mode, downcounting ,PWM 8-bit 
...
void LOW_LEVEL(bool al, bool bl, bool cl) {
  // If xh is 0, then the pin is normal port
  // If xh is 1, then the pin is compare match noninverting mode, downcounting ,PWM 8-bit
  PORTD = al ? (PORTD | 0x02) : (PORTD & ~0x02);   //Pin 2(PD1)
  PORTD = bl ? (PORTD | 0x10) : (PORTD & ~0x10);   //Pin 4(PD4)
  PORTD = cl ? (PORTD | 0x80) : (PORTD & ~0x80);   //Pin 6(PD7)
}

void HIGH_MODE(bool ah, bool bh, bool ch) {
  // If xh is 0, then the pin is normal port
  // If xh is 1, then the pin is compare match noninverting mode, downcounting ,PWM 8-bit
  TCCR0A = ah ? 0x21 : 0;   //Pin 3(OC0B)
  TCCR3A = bh ? 0x81 : 0;   //Pin 5(OC3A)
  TCCR1A = ch ? 0x81 : 0;   //Pin 9(OC1A)
}
...
void SET_PWM(byte width_value){
  ...
  OCR0B  = width_value;                   // Set pin 3  PWM duty cycle
  OCR3A  = width_value;                   // Set pin 5 PWM duty cycle
  OCR1A  = width_value;                   // Set pin 9 PWM duty cycle
}
